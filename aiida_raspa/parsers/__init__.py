...
def parse_block1(fl, result_dict, prop, value=1, units=2, dev=4):
    ...
    for line in fl:
        if 'Average' in line:
            ...
            result_dict[prop+'_units'] = line.split()[units].translate(None, '[](){}')
        ...
        with open(output_abs_path, "r") as f:
            ...
            for line in f:
                for parse in block1_list:
                    if parse[0].match(line):
                        ...
                        for i, cmpnt in enumerate(component_names):
                            ...
                            line = f.next()
                            if cmpnt in line:
                                parse_block1(f, res_per_component[i], parse[1], *parse[2])
                            else:
                                break
                ...
                if framework_density.match(line) is not None:
                    ...
                    result_dict['framework_density_units'] = line.split()[3].translate(None, '[](){}')
